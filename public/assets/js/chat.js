                    ...
                    .joining((user) => {
                        ...
                        document.querySelectorAll(".friends").forEach(function (el) {
                            if (el.getAttribute("data-id") == user.id) {
                                el.querySelector(".friends-credent > .friend-status").innerHTML = "<p class='color:green'>Online</p>"
                            }
                        })
                    })
                    .leaving((user) => {
                        console.log(`${user.name} leaving the room`);
                    })
                    .error((error) => {
                        console.error(error);
                    })
        ...
        axios.get(url)
            .then(function (res) {
                ...
                if (data.length > 0) {
                    ...
                    document.querySelector(".chat-area").innerHTML = "<p style='text-align: center'>Tidak memiliki Pesan</p>"
                }
            })
}
